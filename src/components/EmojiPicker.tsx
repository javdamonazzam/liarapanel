...
import React, { JSX, PromiseLikeOfReactNode, ReactNode, useState } from 'react';
// @mui
...
export default function EmojiPicker({ disabled, value, setValue, alignRight = false, ...other }: propTypes) {
  const theme = useTheme();
  const [emojiPickerState, SetEmojiPicker] = useState(false);

  let emojiPicker: React.ReactNode;
  if (emojiPickerState) {
    emojiPicker = (
      <Picker
        ...
        emoji="point_up"
        data={data}
        onSelect={(emoji: { native: string }) => setValue(value + emoji?.native)}
      />
    );
  }

  const triggerPicker = (event: { preventDefault: () => void }) => {
