...
export default function MailItem({ mail, isDense, isSelected, onSelect, onDeselect, ...other }: propTypes) {
  ...
  return (
    ...
    >
      ...
      <Link href={linkTo(params, mail.id)}>
              ...
              <>
                <Box sx={{ display: 'flex' }}>
                  {mail.labelIds.map((labelId) => {
                    const label = [].find((_label) => _label.id === labelId);
